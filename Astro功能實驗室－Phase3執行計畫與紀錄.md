...
- 建立日期：2026-07-20
- 目前狀態：Step 1 F11、Step 2 F16 已完成；下一步為 Step 3 F02 網站內桌寵規劃
...
- 2026-07-21 已以 Cubism SDK for Web 5-r.5 建立 `/lab/character-animation/`；不新增 npm dependency，原始 ZIP 不提交。Miara 最初通過初步瀏覽器 QA，但使用者認為風格不符，已改採官方 `koharu_haruto_ja` 配布包中的 Koharu。新模型提供 Idle／Tap／FlickLeft，2048px 貼圖約 1.43 MB；Haruto 暫不納入。使用者已確認 Koharu 風格、比例、持續待機與三組控制符合概念測試需求；共用偏好亦新增「完整動態」，並分開顯示設定值與解析結果。
...
開始 Step 3・F02 網站內桌寵規劃。優先沿用 Step 2 的 Live2D runtime，先定義桌寵的頁面內活動範圍、開關／暫停、不遮內容、手機與低效能降級及 cleanup 停止點；未經使用者核准不新增第二套人物 runtime 或其他 dependency。
