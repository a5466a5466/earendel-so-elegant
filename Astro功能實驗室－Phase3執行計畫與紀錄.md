...
- 建立日期：2026-07-20
- 目前狀態：Step 1 F11 已完成；Step 2 F16 Prototype 已通過使用者人工驗收，等待剩餘技術 QA
...
| 2 | F16 人物動畫／Live2D 類效果 | Step 1、F26 | Prototype 與人工驗收通過；等待剩餘技術 QA |
...
- 2026-07-21 已以 Cubism SDK for Web 5-r.5 建立 `/lab/character-animation/`；不新增 npm dependency，原始 ZIP 不提交。Miara 最初通過初步瀏覽器 QA，但使用者認為風格不符，已改採官方 `koharu_haruto_ja` 配布包中的 Koharu。新模型提供 Idle／Tap／FlickLeft，2048px 貼圖約 1.43 MB；Haruto 暫不納入。使用者已確認 Koharu 風格、比例、持續待機與三組控制符合概念測試需求；共用偏好亦新增「完整動態」，並分開顯示設定值與解析結果。Step 2 尚待 production build、錯誤 fixture 與完整生命週期 QA。
...
繼續 Step 2・F16 人物動畫／Live2D 類效果。Koharu Prototype 與人工驗收已通過；下一個動作是排除 production build 的 `picomatch` 模組載入問題，完成 768／1440 px、錯誤 fixture 與反覆載入／cleanup QA。全部通過前不標記 Step 2 正式結案，也不進入 Step 3。
