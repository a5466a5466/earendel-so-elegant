...
- 建立日期：2026-07-20
- 目前狀態：Step 1 F11 已完成技術 QA 與使用者人工驗收；Step 2 F16 已完成概念驗證規劃，等待 SDK／範例模型授權與下載閘門
...
| 2 | F16 人物動畫／Live2D 類效果 | Step 1、F26 | 已完成概念驗證規劃；等待授權與 SDK 取得 |
...
繼續 Step 2・F16 人物動畫／Live2D 類效果。下一個動作是由使用者閱讀並接受適用的 Live2D SDK 與官方範例模型條款，再取得 Cubism SDK for Web 配布包及一個可用於 SDK 整合測試的官方原創範例模型。條款未確認、檔案未核對前，不把任何 SDK、Cubism Core 或模型加入 repository，也不安裝其他非官方 Live2D wrapper。
