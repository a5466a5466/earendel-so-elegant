...
- 建立日期：2026-07-20
- 最後更新：2026-07-20
- 目前狀態：Step 4 與 Phase 2 已完成技術 QA、使用者最終人工驗收並結案
...
| 3 | F01 動畫滑鼠 | 完成並由使用者驗收 |
| 4 | Phase 2 整合 QA 與結案 | 完成並由使用者驗收；Phase 2 結案 |
...
- 路由：`/lab/cursor/`。
- 可直接比較原生游標、32 × 32 靜態星芒 placeholder 與保留原生指標的動態星環。
- 動態星環只有一個 `pointer-events: none` DOM 元素；四顆白色星點各自在不同半徑與週期繞行，兩顆順時針、兩顆逆時針，其中兩顆使用四向星芒；各星點有不同週期的亮度起伏，外圈以陰影明暗呼吸，不建立拖尾或粒子陣列。
- 精準滑鼠與 Hover 能力、共用動態偏好、共用效能偏好共同決定是否啟用。
- 觸控、無 Hover、reduced motion 與 economy 安全回到原生游標。
- 無額外 dependency。

### 技術 QA

- `pnpm build:lab` 通過，共 34 個頁面。
- `pnpm build` 通過；正式輸出無 `/lab/`、`/lab-assets/` 或 cursor bundle。
- 瀏覽器確認三模式切換、星環跟隨、操作穿透、輸入與按鈕正常。
- reduced motion 與 economy 均會立即隱藏星環並切回 native。
- 360／768／1280 px 無水平溢位；Console 無 error 或 warning。

詳細紀錄：`Astro功能實驗室－Phase2-Step3-F01動畫滑鼠.md`。

### 使用者決策

2026-07-20 使用者驗收最終動態星環版本：四顆白色星點採不同半徑與週期獨立繞行，兩顆順時針、兩顆逆時針，其中兩顆為四向星芒；保留不同亮度週期與外圈呼吸。整組等速旋轉及整組變速方案均不採用。Step 3 正式完成，結論為條件採用。
...
- `pnpm build:lab` 與 `pnpm build` 通過；正式輸出沒有 `/lab/`、`/lab-assets/`、Lab 命名 bundle 或 Sitemap `/lab/` 參照。
- 32 個 Lab HTML 的 noindex、共用導覽、重複 ID 與內部連結守門全部通過。
- 360／768／1440 px 的 Lab 首頁、X／Threads、YouTube 與動畫游標共 12 組路由檢查均無水平溢位。
- 手機選單開關與焦點返回、YouTube 單一播放器切換、社群官方 iframe、動畫游標輸入語意、reduced motion／economy 降級與跨頁偏好均通過。
- `pointer: coarse`／`hover: none` 的程式與 CSS 雙重守門已確認；實體觸控、Safari／Firefox、螢幕閱讀器與正式部署效能依既有決策留到正式整合／部署階段。
- Console 無 error 或 warning；Phase 2 無新增 dependency。

詳細紀錄：`Astro功能實驗室－Phase2-Step4-QA與結案.md`。

### 使用者決策

2026-07-20 使用者完成 Phase 2 最終人工驗收，接受 X／Threads 多貼文版面、YouTube 共存與條件式動態星環的目前版本，並同意實體觸控、跨瀏覽器、螢幕閱讀器與正式內容效能留到正式整合／部署階段。Step 4 與 Phase 2 正式結案。
...
Phase 2 已完成並由使用者驗收結案。後續若開始正式網站整合，應先依 Step 4 紀錄確認正式社群貼文、核准品牌游標素材與部署 QA 範圍；最新 10 筆自動同步等取得必要授權再另行規劃。
