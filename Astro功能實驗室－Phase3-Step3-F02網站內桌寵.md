...
## 9. 公開展示易用性調整（2026-07-23）

使用者在公開 Lab 示範流程中指出，原本固定於右下角的小型「叫出桌寵」按鈕不夠明顯，且大螢幕上的長距離遊走會因 4.3 秒 duration 上限而產生過快位移。因此完成以下調整：

- 啟動入口移入頁面介紹區中央，改為最大 440px、最小高度 92px 的大型 CTA，包含「叫出 Koharu 桌寵」與載入後行為說明。
- CTA 保留鍵盤焦點、hover 回饋與手機寬度適配；桌寵載入後入口消失，仍維持單一 Canvas／renderer。
- 遊走速度上限固定為每秒 125px；移除 4.3 秒 duration 上限，長距離移動改為依距離延長時間，最短仍為 1.8 秒。
- `pnpm build:lab` 再次通過 39 頁；瀏覽器確認 CTA 為頁面內靜態元素、尺寸 440×92px，桌寵載入與遊走狀態正常，Console error／warning 為 0。
