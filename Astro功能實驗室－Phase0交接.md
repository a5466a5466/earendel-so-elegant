...
> **Phase 0・Step 1、Step 2、Step 3、Step 4 已完成，目前停在 Step 5「搜尋引擎與部署隔離」開始前。**
...
| 5 | 搜尋引擎與部署隔離 | **部分完成** | 下一個要執行的步驟；`noindex` 已完成 |
...
## 7. Step 3、Step 4 已完成成果與 Step 5～6 未來工作
...
```text
分支：master
HEAD：1602f21 phase 0 step 3 done
遠端：origin/master 仍停在 ae1ae42，本機領先 3 個 commit
工作樹：有 Step 4 共用結構與本交接檔的未提交變更
```

因此 Step 1 與上一版交接檔已在遠端；Step 2、Step 3 已 commit 但尚未 push；Step 4 尚未 commit。

本次 Step 4 新增或修改的共用資料、Hero、Layout、頁面、樣式與交接紀錄，在 commit 並 push 前仍只存在目前電腦。
...
10. 下一步先規畫 Step 5，不要直接開始功能實驗。
11. 每完成一步都要讓使用者實際查看並驗收。
