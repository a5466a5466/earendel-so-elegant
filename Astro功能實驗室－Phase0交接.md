...
> **Phase 0・Step 1～Step 4 已完成；Step 5「搜尋引擎與部署隔離」已完成開發與雙模式 build 驗證，正在等待使用者確認公開策略。**
...
Step 5 已實作「開發模式保留、正式 build 預設排除」策略。一般 `pnpm build` 會隱藏首頁入口並移除 `dist/lab/`；`pnpm build:lab` 會保留入口與 Lab 頁面供驗收。兩種建置皆已通過技術驗證，目前等待使用者確認此策略。
...
| 5 | 搜尋引擎與部署隔離 | **待使用者驗收** | 雙模式 build 與 Sitemap 防呆已完成，等待策略確認 |
...
10. Step 5 已完成開發與雙模式 build 驗證，先確認是否通過使用者策略驗收。
11. Step 5 通過後才執行 Step 6 完整 QA，不要直接開始功能實驗。
12. 每完成一步都要讓使用者實際查看並驗收。
