...
| 分支 | `master` |
| HEAD | `4abb639` Phase 3・Step 2 Koharu Prototype checkpoint；其後 final QA 修改待提交，現況以 Git 核對 |
| 遠端 | 採 `master` 直接推送至 `origin/master`；同步狀態以 `git status -sb` 核對 |
| 工作樹 | Phase 3・Step 2 final QA 與結案紀錄已完成、目前待提交；現況以 `git status -sb` 為準 |
| Phase 0 | Step 1～6 完成 |
| Phase 1 | Step 1～18 全部完成；2026-07-20 通過使用者驗收並結案 |
| Phase 2 | Step 1、Step 3、Step 4 已完成使用者驗收並結案；Step 2 最新列表暫緩 |
| Phase 3 | Step 1 F11、Step 2 F16 已完成；Cubism SDK for Web 通過 Koharu Prototype、人工驗收、錯誤降級、RWD、生命週期與正式輸出隔離 QA |
| Phase 4 | 尚未開始；F24 投稿表單／後端評估、F25 PWA |
| 下一步 | 規劃 Step 3・F02 網站內桌寵；沿用 Step 2 Live2D runtime，不先新增第二套引擎 |
...
2026-07-21 Step 2 第一階段 Prototype 已實作於 `/lab/character-animation/`。Miara 最初通過真實 WebGL、動作、reduced-motion 與 360px 初測，但使用者認為風格不符，隨後改用官方 `koharu_haruto_ja.zip` 中的 Koharu。新模型使用 2048px／約 1.43 MB 貼圖，提供 Idle／Tap／FlickLeft；同包 Haruto 暫不納入。原始 ZIP 均由 `.gitignore` 排除，專案沒有新增 npm dependency。使用者已完成人工驗收，確認 Koharu 風格、比例、持續待機與動作控制符合概念需求；共用偏好新增「完整動態」，Live2D 卡片分開顯示設定與解析結果。
