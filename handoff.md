...
| 分支 | `master` |
| HEAD | 本文件所在提交完成 Phase 3・Step 3；精確 hash 以 `git log -1 --oneline` 核對 |
| 遠端 | 採 `master` 直接推送至 `origin/master`；同步狀態以 `git status -sb` 核對 |
| 工作樹 | Step 3 交付後應為乾淨；現況以 `git status -sb` 為準 |
| Phase 0 | Step 1～6 完成 |
| Phase 1 | Step 1～18 全部完成；2026-07-20 通過使用者驗收並結案 |
| Phase 2 | Step 1、Step 3、Step 4 已完成使用者驗收並結案；Step 2 最新列表暫緩 |
| Phase 3 | Step 1 F11、Step 2 F16、Step 3 F02 已完成使用者人工驗收 |
| Phase 4 | 尚未開始；F24 投稿表單／後端評估、F25 PWA |
| 下一步 | 開始 Step 4・F21 粒子、視差與 WebGL；先規劃 CSS／Canvas 基線、效能與降級條件 |
