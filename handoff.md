...
| 分支 | `master` |
| HEAD | `dd14bc2 phase 1 step 18 done` |
| 遠端 | 本機 `master` 比 `origin/master` 領先 1 個 commit；Phase 1 結案尚未 push |
| 工作樹 | 有未提交變更：Phase 1 結案文件、Phase 2 執行計畫與交接修正 |
| Phase 0 | Step 1～6 完成 |
| Phase 1 | Step 1～18 全部完成；2026-07-20 通過使用者驗收並結案 |
| Phase 2 | 執行計畫已建立；Step 1～5 尚未開始 |
| 下一步 | 取得使用者核准後開始 Phase 2・Step 1 外部內容 facade、同意與 fixture 契約 |
...
3. `Astro功能實驗室－Phase1-Step18-QA與結案.md`（Phase 1 最終基線）。
4. 最新已完成 Step 與準備開始 Step 的紀錄文件。
...
Phase 2 執行計畫已於 2026-07-20 建立，詳細內容位於 `Astro功能實驗室－Phase2執行計畫與紀錄.md`。規劃共五步：共用外部內容 facade／同意契約、X、Threads、動畫滑鼠、整合 QA 與結案。

2026-07-20 官方查核確認 X embed 會涉及頁面、IP、瀏覽器與 Cookie 等資訊，因此固定採用點擊後才連線；Threads 網頁已遷移至 `threads.com`，而官方開發文件當下受存取限制，Step 3 必須先重新做官方能力探測，不預設舊 oEmbed／embed script 仍是有效契約。

下一步需由使用者核准 Phase 2・Step 1「外部內容 facade、同意與 fixture 契約」。Step 1 不需要真實 X／Threads 素材、不新增 dependency，也不會連線第三方；它只固定兩個平台共用的靜態卡片、同意粒度、loader 去重、逾時、fallback 與 cleanup 契約。
