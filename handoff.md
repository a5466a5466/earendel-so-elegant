...
2026-07-21 Step 2 第一階段 Prototype 已實作於 `/lab/character-animation/`。Miara 最初通過真實 WebGL、動作、reduced-motion 與 360px 初測，但使用者認為風格不符，隨後改用官方 `koharu_haruto_ja.zip` 中的 Koharu。新模型使用 2048px／約 1.43 MB 貼圖，提供 Idle／Tap／FlickLeft；同包 Haruto 暫不納入。原始 ZIP 均由 `.gitignore` 排除，專案沒有新增 npm dependency。使用者已完成人工驗收，確認 Koharu 風格、比例、持續待機與動作控制符合概念需求；共用偏好新增「完整動態」，Live2D 卡片分開顯示設定與解析結果。`pnpm build:lab` 與瀏覽器 QA 通過；正式 build 仍受 Astro／Vite 的 `picomatch`／`require is not defined` 問題阻擋，另待錯誤 fixture、768／1440 與反覆載入／cleanup QA。完成前不進入 Step 3。詳細證據位於 `Astro功能實驗室－Phase3-Step2-F16-Live2D概念驗證.md`。
