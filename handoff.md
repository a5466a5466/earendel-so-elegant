...
| Phase 2 | Step 1 X／Threads 官方直接嵌入已技術完成 |
| 下一步 | 視需要替換正式貼文；否則開始 F01 動畫滑鼠 |
...
Phase 2 執行計畫已於 2026-07-20 建立，詳細內容位於 `Astro功能實驗室－Phase2執行計畫與紀錄.md`。使用者後續決定簡化外部內容：X／Threads 合併成一個官方直接嵌入 Step，之後是正式貼文替換、動畫滑鼠與整合 QA。
...
Phase 2・Step 1 已於 2026-07-20 完成技術實作，Prototype 位於 `/lab/social-embeds/`。它直接使用 X 官方文件的 `@Interior` 範例與 Threads 已驗證官方帳號公開貼文，各載入一份官方 script；沒有自製同意視窗、sessionStorage、failure fixture 或額外 dependency。瀏覽器已確認兩個官方 iframe 都成功產生，1280 px 無水平溢位。詳細證據位於 `Astro功能實驗室－Phase2-Step1-X與Threads官方嵌入.md`。

目前不需要使用者提供貼文網址。正式內容準備好時，只需提供一筆 X 與一筆 Threads 公開貼文 URL 進行替換；若暫不替換，下一個新功能是 F01 動畫滑鼠。
