	...
	return {
		...
		hooks: {
			...
			'astro:build:done': async ({ dir, logger }) => {
				...
				const labAssetPattern = /^(?:LabLayout\.|LabControls\.|LabNavigation\.|preferences(?:\.astro[^.]*)?\.|navigation(?:\.astro[^.]*)?\.|scroll(?:\.astro[^.]*)?\.|carousel(?:\.astro[^.]*)?\.|lightbox(?:\.astro[^.]*)?\.|video(?:\.astro[^.]*)?\.|youtube(?:\.astro[^.]*)?\.|audio(?:\.astro[^.]*)?\.|sound-effects(?:\.astro[^.]*)?\.|share(?:\.astro[^.]*)?\.|audio-manager\.|gallery-|starlight-birthday\.|summer-letter\.|moonlit-message-transparent\.)/;
